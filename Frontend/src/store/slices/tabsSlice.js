...
const tabsSlice = createSlice({
    ...
    reducers: {
        openTab: (state, action) => {
            const existing = state.allTabs.find(t => t.objectId === action.payload.objectId && t.type === action.payload.type);
            if (!existing) {
                ...
                if (action.payload.hierarchyPath)
                    existing.hierarchyPath = action.payload.hierarchyPath;
            }
        ...
        closeOthers: (state, action) => {
            state.allTabs = state.allTabs.filter(t => t.id === action.payload || t.isPinned);
        ...
        updateTab: (state, action) => {
            ...
            if (action.payload.name !== undefined)
                tab.objectName = action.payload.name;
            if (action.payload.hierarchyPath !== undefined)
                tab.hierarchyPath = action.payload.hierarchyPath;
        },
    },
});
export const { openTab, closeTab, closeOthers, closeAll, setActiveTab, markTabUnsaved, markTabSaved, pinTab, unpinTab, restoreLastClosed, updateTab, } = tabsSlice.actions;
