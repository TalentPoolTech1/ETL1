...
const pipelineSlice = createSlice({
    ...
    reducers: {
        setPipeline: (state, action) => {
            ...
            state.nodes = action.payload.nodes.reduce((acc, node) => ({ ...acc, [node.id]: node }), {});
            state.edges = action.payload.edges.reduce((acc, edge) => ({ ...acc, [edge.id]: edge }), {});
            state.unsavedChanges = false;
        },
        updateActivePipeline: (state, action) => {
...
export const { setPipeline, updateActivePipeline, addNode, updateNode, deleteNode, addEdge, deleteEdge, selectNode, clearSelection, markSaved, } = pipelineSlice.actions;
