...
import FunctionMatrixService, { FeatureFlagConfig } from './FunctionMatrixService';
...
export class FunctionMatrixDevTools {
  private service: FunctionMatrixService;

  constructor(service?: FunctionMatrixService) {
   ...
   */
  listCategory(category: FunctionCategory): void {
   ...
   */
  listTechnology(tech: SourceTechnology): void {
   ...
   */
  showFunction(functionId: string): void {
    ...
    for (const tech of this.service.getAllPlusPySpark()) {
      const cap = fn.capabilities[tech as SourceTechnology];
      if (cap) {
        console.log(`  ${tech}:`);
        console.log(`    Support: ${cap.support}`);
        console.log(`    Syntax: ${cap.syntax}`);
        if (cap.notes) console.log(`    Notes: ${cap.notes}`);
        if (cap.example) console.log(`    Example: ${cap.example}`);
      }
    }
    console.log('===========================================\n');
  }
   ...
   */
  toggleCategory(category: FunctionCategory): boolean {
   ...
   */
  toggleFunction(functionId: string): boolean {
    const flags = this.service.getFeatureFlags();
   ...
   */
  getSyntax(functionId: string, tech: SourceTechnology): string | null {
    const fn = this.service.getFunction(functionId);
    if (!fn) return null;
   ...
   */
  validateMatrix(): MatrixValidationReport {
    const errors: string[] = [];
    ...
    if (stats.totalFunctions === 0) {
      ...
      return { isValid: false, errors, warnings, stats: stats as any };
    }
    ...
    const coverageByTech: { [key: string]: { native: number; alternative: number; partial: number; none: number } } = {};
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  (window as any).functionMatrixDev = functionMatrixDevTools;
  console.log(
    'Function Matrix Dev Tools available as window.functionMatrixDev'
  );
  console.log(
    'Usage: functionMatrixDev.printCurrentState(), functionMatrixDev.listCategory(category), etc.'
  );
}
