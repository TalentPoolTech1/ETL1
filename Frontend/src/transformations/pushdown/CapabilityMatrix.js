 ...
 */
export function hasAlternative(primitiveId, technology) {
    const cap = getCapability(primitiveId, technology);
 ...
 */
export function getUnsupportedFunctions(technology) {
    return Object.keys(CAPABILITY_MATRIX).filter(primitiveId => getCapability(primitiveId, technology)?.availability === 'none');
}
/**
 * Get all primitives available natively for a source technology
 */
export function getNativeFunctions(technology) {
    return Object.keys(CAPABILITY_MATRIX).filter(primitiveId => getCapability(primitiveId, technology)?.availability === 'native');
}
/**
 * Get all primitives that are available (native or alternative) for a source technology
 */
export function getAvailableFunctions(technology) {
    return Object.keys(CAPABILITY_MATRIX).filter(primitiveId => getCapability(primitiveId, technology)?.availability !== 'none');
}
/**
 * Get functions available only in PySpark (not in any source DB)
 */
export function getPySparkOnlyFunctions() {
    const sourceDbTechs = ['oracle', 'postgresql', 'mysql', 'sql_server', 'redshift', 'snowflake'];
    return Object.keys(CAPABILITY_MATRIX).filter(primitiveId => {
        const allUnsupported = sourceDbTechs.every(tech => getCapability(primitiveId, tech)?.availability === 'none');
