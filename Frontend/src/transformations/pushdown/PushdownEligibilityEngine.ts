...
import { SourceTechnology, isNativeFunctionSupport, getCapability } from './CapabilityMatrix';

export type LineageOrigin = 'SOURCE_DIRECT' | 'PYSPARK_DERIVED' | 'MIXED_UPSTREAM';
export type ExecutionPoint = 'source' | 'pyspark' | 'forced_pyspark';

/**
 * Information about a source table in the pipeline
 */
...
export interface ColumnLineage {
  ...
  sourceTable?: SourceTableInfo; // If SOURCE_DIRECT
  sourceColumnName?: string;
  transformedAt: string[]; // stepIds that modified this column
  pysparkIntroducedAt?: string; // stepId where it became PYSPARK_DERIVED
}

/**
 * Eligibility result for a segment
 */
export interface SegmentEligibility {
  segmentId: string;
  stepRange: string; // "Step 1-5"
  eligible: boolean;
  reasons: string[];
  ineligibilityReasons?: string[];
  ineligibilityType?: 'function_unsupported' | 'cross_source' | 'lineage_break' | 'mixed_sources';
...
/**
 * Full eligibility analysis result
 */
export interface EligibilityAnalysis {
  segments: SegmentEligibility[];
  columnLineage: Map<string, ColumnLineage>;
  executionBoundaries: Array<{ before: string; after: string; reason: string }>;
  crossSourceJoins: Array<{ stepId: string; tech1: SourceTechnology; tech2: SourceTechnology }>;
}
...
export class PushdownEligibilityEngine {
  private sourceTablesMap: Map<string, SourceTableInfo> = new Map();
  private columnLineageMap: Map<string, ColumnLineage> = new Map();
  private pysparkSteps: Set<string> = new Set();
  private transformRegistry: Map<string, { supportedIn: SourceTechnology[] }>;
  ...
  public analyze(
    ...
    userExecutionPointChoices: Map<string, ExecutionPoint> = new Map()
  ): EligibilityAnalysis {
    // Initialize
   ...
   */
  public areInSameSegment(step1: TransformStep, step2: TransformStep): boolean {
   ...
   */
  private propagateLineageBreaks(sequence: TransformSequence): void {
   ...
   */
  private analyzeSegments(sequence: TransformSequence): SegmentEligibility[] {
    const segments: SegmentEligibility[] = [];
    ...
    for (let i = 0; i < sequence.steps.length; i++) {
      ...
      if (!canContinueSegment || isLastStep) {
        ...
        if (segmentEnd >= currentSegmentStart) {
          const segEligibility = this.analyzeSegmentSteps(
            sequence.steps.slice(currentSegmentStart, segmentEnd + 1),
            currentSegmentStart,
            segmentEnd
          );
  ...
  private analyzeSegmentSteps(
    ...
    const tech = sourceTechs[0] as SourceTechnology | undefined;
    ...
    const suggestedExecutionPoint: ExecutionPoint = eligible ? 'source' : 'forced_pyspark';
    ...
    return {
      ...
      affectedColumns,
      sourceTechnologies: sourceTechs as SourceTechnology[],
    };
  }
   ...
   */
  private isStepEligibleForPushdown(step: TransformStep): boolean {
   ...
   */
  private detectSourceTechnologies(steps: TransformStep[]): string[] {
   ...
   */
  private getStepInputColumns(step: TransformStep): string[] {
   ...
   */
  private getStepOutputColumns(step: TransformStep): string[] {
   ...
   */
  private getSegmentColumns(steps: TransformStep[]): string[] {
   ...
   */
  private detectBoundaries(segments: SegmentEligibility[]): Array<{ before: string; after: string; reason: string }> {
    const boundaries: Array<{ before: string; after: string; reason: string }> = [];
   ...
   * Detect cross-source joins
   */
  private detectCrossSourceJoins(sequence: TransformSequence): Array<{ stepId: string; tech1: SourceTechnology; tech2: SourceTechnology }> {
    const joins: Array<{ stepId: string; tech1: SourceTechnology; tech2: SourceTechnology }> = [];
   ...
   */
  private determineIneligibilityType(steps: TransformStep[]): 'function_unsupported' | 'cross_source' | 'lineage_break' | 'mixed_sources' | undefined {
    // Check for each issue type
    for (const step of steps) {
      if (step.type === 'join') return 'cross_source';
    }

    for (const col of this.columnLineageMap.values()) {
      if (col.origin === 'PYSPARK_DERIVED') return 'lineage_break';
    }

    const techs = this.detectSourceTechnologies(steps);
    if (techs.length > 1) return 'mixed_sources';
 ...
 */
export function createEligibilityEngine(): PushdownEligibilityEngine {
