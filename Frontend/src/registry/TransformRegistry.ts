...
export interface TransformPrimitive {
  id: string; // Technical ID (e.g., 'trim_timestamp')
  label: string; // Plain-language label shown in UI
  ...
  // Code generation templates per engine
  codeGenTemplate: {
  ...
  // Sample input/output for UI demonstration
  sample: {
...
export const TRANSFORM_REGISTRY: Record<string, TransformPrimitive> = {
  ...
  date_add: {
    ...
    codeGenTemplate: {
      spark: (params, input) =>
        params.unit === 'MONTH' ? `ADD_MONTHS(${input}, ${params.amount})` : `DATE_ADD(${input}, ${params.amount})`,
