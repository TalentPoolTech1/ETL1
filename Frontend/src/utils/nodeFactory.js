 ...
 */
export function createSourceNode(connectorId, schema, table, opts = {}) {
    return {
        id: uuid(),
 ...
 */
export function createTransformNode(name = 'Transform', opts = {}) {
    return {
        id: uuid(),
        name,
        type: 'transform',
 ...
 */
export function createAggregationNode(groupByColumns = [], opts = {}) {
    return {
        id: uuid(),
        name: 'Aggregation',
        type: 'aggregation',
 ...
 * Creates a join node
 */
export function createJoinNode(leftTable = '', rightTable = '', joinType = 'INNER', opts = {}) {
    return {
        id: uuid(),
        name: `${joinType} Join`,
        type: 'join',
 ...
 */
export function createTargetNode(connectorId, table, writeMode = 'OVERWRITE', opts = {}) {
    return {
        id: uuid(),
        name: `Write to ${table}`,
        type: 'target',
 ...
 */
export function createUnionNode(unionType = 'UNION', opts = {}) {
    return {
        id: uuid(),
        name: 'Union',
        type: 'union',
 ...
 */
export function createCustomSQLNode(sql = '', opts = {}) {
    return {
        id: uuid(),
        name: 'Custom SQL',
        type: 'custom_sql',
 ...
 */
export function getNextNodeType(currentType) {
    const nextMap = {
 ...
 */
export function calculateAutoPosition(existingNodes) {
    ...
    if (targetNodes.length > 0) {
        ...
        const rightmost = Math.max(...existingNodes.filter(n => n.type !== 'target').map(n => n.x + n.width));
 ...
 */
export function createNodeFromMetadataItem(item, connectorId, schema, position) {
    if (item.type !== 'table') {
        return null;
    }
