 ...
 */
export function serializePipelineToDefinition(pipeline, nodes, edges, options = {}) {
    // Build target-id → [source-id, ...] input map from edges
    const inputsMap = {};
    Object.values(edges).forEach(e => {
        if (!inputsMap[e.target])
            inputsMap[e.target] = [];
        inputsMap[e.target].push(e.source);
    });
    const backendNodes = Object.values(nodes).map(n => serializeNode(n, inputsMap[n.id] ?? []));
    return {
        id: pipeline.id,
        name: pipeline.name,
        ...
        environment: {
            ...
            sparkVersion: (options.sparkVersion ?? '3.5'),
        },
        sparkConfig: {
...
function serializeNode(node, inputs) {
