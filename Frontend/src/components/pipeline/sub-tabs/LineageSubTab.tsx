...
export function LineageSubTab({ pipelineId }: Props) {
  ...
  let ledges: { from: string; to: string }[] = [];
              ...
              return (
                <g key={n.id} onClick={() => setSelected(n.id)} className="cursor-pointer">
                  <rect x={x - 60} y={y - 22} width={120} height={44} rx={6}
                    fill={fill} stroke={isActive ? '#1d4ed8' : 'transparent'} strokeWidth={2} />
                  <text x={x} y={y + 5} textAnchor="middle" fontSize={12} fill={textFill} fontWeight={n.isCurrent ? 600 : 400}>
                    {n.label.length > 14 ? n.label.slice(0, 13) + '…' : n.label}
                  </text>
                  <text x={x} y={y + 20} textAnchor="middle" fontSize={9} fill={textFill} opacity={0.75}>{n.kind}</text>
                </g>
              );
            })}
            {ledges.map((e, i) => {
              ...
              return <line key={i} x1={fromX} y1={120} x2={toX} y2={120} stroke="#94a3b8" strokeWidth={1.5} markerEnd="url(#arrow)" />;
            })}
            {lnodes.length === 1 && (
