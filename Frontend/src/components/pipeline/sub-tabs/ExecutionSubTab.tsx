    ...
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="w-72 flex-shrink-0 flex flex-col border-r border-neutral-200 overflow-hidden">
        <div className="p-4 border-b border-neutral-200 space-y-3">
          ...
          <div className="space-y-2">
            <div>
              ...
              <select value={environment} onChange={e => setEnvironment(e.target.value)}
                ...
                className="w-full mt-1 px-2 py-1.5 border border-neutral-300 rounded text-sm bg-white">
          ...
          <div className="flex gap-2">
            {runState !== 'running' ? (
              <Button className="flex-1" onClick={startRun}>▶ Run</Button>
            ) : (
              <Button className="flex-1" variant="ghost" onClick={stopRun}>⏹ Stop</Button>
            )}
          </div>
          ...
          {(runState === 'success' || runState === 'failed') && (
            <div className={`text-xs text-center py-1 rounded ${runState === 'success' ? 'bg-success-50 text-success-700' : 'bg-danger-50 text-danger-700'}`}>
              {runState === 'success' ? `Completed in ${fmtElapsed(elapsed)}` : 'Run stopped or failed'}
            </div>
          )}
        </div>
              ...
              {steps.map((step, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {step.duration && <div className="text-xs text-neutral-400">{step.duration}</div>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-neutral-950">
        <div className="flex items-center justify-between px-4 py-2 border-b border-neutral-800 flex-shrink-0">
          ...
          <button onClick={() => setLogs([])} className="text-xs text-neutral-500 hover:text-neutral-300">Clear</button>
        </div>
        <div ref={logRef} className="flex-1 overflow-y-auto p-4 font-mono text-xs text-neutral-200 space-y-0.5">
          {logs.length === 0 ? (
          ...
          {runState === 'running' && <div className="text-primary-400 animate-pulse">▌</div>}
        </div>
      </div>
    </div>
  );
