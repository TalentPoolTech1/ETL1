...
export function OrchestratorPermissionsSubTab({ orchId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [permissionsResponse, usersResponse, rolesResponse] = await Promise.all([
        api.getOrchestratorPermissions(orchId),
        api.getUsers(),
        api.getRoles(),
      ]);
  ...
  const removeGrant = async (id: string) => {
    const updated = grants.filter(grant => grant.id !== id);
    setGrants(updated);
    await persist(updated);
  };
      ...
      <div className="flex items-center justify-between p-4 bg-neutral-50 border border-slate-800 rounded-lg">
        <div>
          ...
          <div className="text-xs text-neutral-500 mt-0.5">
            {!projectScoped
              ? 'Global orchestrators are not tied to a project, so project grants do not apply.'
              : 'Orchestrator access is project-scoped. Changes here update project member roles.'}
          </div>
        </div>
        <button
          type="button"
                ...
                {grants.map(g => (
                  <tr key={g.id} className="hover:bg-neutral-50">
                    ...
                    <td className="px-4 py-2">
                      <select
                        value={g.roleId}
                        onChange={e => { void changeRole(g.id, e.target.value); }}
                    ...
                    <td className="px-4 py-2 text-right">
                      <button
                        ...
                        disabled={!projectScoped}
                        className="text-neutral-400 hover:text-danger-600 text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Remove
                      </button>
                    </td>
