    ...
    <div className="flex flex-col h-full overflow-hidden bg-[#0d0f1a]">
      ...
      <div className="flex-1 overflow-auto min-h-0">
        <table className="w-full border-collapse text-[12px]">
          ...
          <tbody>
            {loading && <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-500 text-sm">Loading…</td></tr>}
            {!loading && filtered.length === 0 && <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-600 text-sm">No runs found.</td></tr>}
              ...
              <tr key={row.orchRunId}
                ...
                <td className="px-3 py-1.5">
                  <button onClick={() => openRun(row.orchRunId, row.orchestratorName ?? 'Orchestrator')}
                    className="font-mono text-[11px] text-blue-400 hover:underline">{row.orchRunId?.slice(0,12)}…</button>
                </td>
                <td className="px-3 py-1.5">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded border text-[11px] font-medium ${STATUS_STYLE[row.runStatus] ?? 'bg-slate-800 text-slate-400 border-slate-700'}`}>
                    {row.runStatus}
                  </span>
                </td>
                <td className="px-3 py-1.5 text-slate-400 text-[11px] whitespace-nowrap">{fmtDt(row.startDtm)}</td>
                <td className="px-3 py-1.5 text-slate-400 text-[11px] whitespace-nowrap">{fmtDt(row.endDtm)}</td>
                <td className="px-3 py-1.5 text-slate-300 font-mono">{fmtDur(row.durationMs)}</td>
                <td className="px-3 py-1.5 text-slate-500 text-[11px] capitalize">{row.triggerType?.toLowerCase()}</td>
                <td className="px-3 py-1.5 text-red-400 text-[11px] truncate max-w-[200px]" title={row.errorMessage ?? ''}>
