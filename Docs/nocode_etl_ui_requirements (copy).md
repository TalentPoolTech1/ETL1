...
# Additional UI Requirement Section: Tab Origin / Object Hierarchy Visibility

## 1. Objective

Every opened tab must clearly display the full hierarchy path from which the object originates. This ensures users always understand the object’s context, especially in deep folder structures and when duplicate names exist.

This is a strict requirement due to:
- no page-based navigation
- multi-tab environment
- deeply nested hierarchy
- reuse of object names across projects

---

## 2. Purpose

The hierarchy path must help users instantly answer:
- Which project does this belong to?
- Which folder/sub-folder contains it?
- Is it global or project-specific?
- Are there duplicates of this object elsewhere?

---

## 3. Display Locations

The full hierarchy must be visible in:

- Object header (mandatory)
- Tab tooltip (mandatory)
- Execution header (mandatory)
- History and audit views (mandatory)
- Search results (mandatory)
- Permissions screen (mandatory)
- Dependency / lineage views (mandatory)
...
## 4. Path Format Standard

Use a consistent arrow-based format:

Examples:

Projects -> Project_A -> Folder_1 -> Pipeline_Customer_Load  
Projects -> Project_A -> Folder_1 -> Subfolder_2 -> Orchestrator_Daily_Run  
Projects -> Global Pipelines -> Global_Pipeline_1  
Connections -> RDBMS -> Postgres -> Finance_DEV  
Metadata -> Cloud -> AWS -> Redshift -> Dev -> Schema1 -> Table1  

Rules:
- Always start from root category (Projects, Connections, Metadata, etc.)
- Maintain left-to-right hierarchy
- Do not truncate in main header
- Use ellipsis only in tooltips if extremely long

---

## 5. Object-Level Path Examples

### Project
Projects -> Project_A

### Folder
Projects -> Project_A -> Folder_1

### Sub-folder
Projects -> Project_A -> Folder_1 -> Subfolder_2

### Pipeline
Projects -> Project_A -> Folder_1 -> Pipeline_X

### Orchestrator
Projects -> Project_A -> Folder_1 -> Orchestrator_X

### Global Pipeline
Projects -> Global Pipelines -> Pipeline_X

### Global Orchestrator
Projects -> Global Orchestrators -> Orchestrator_X

### Connection
Connections -> RDBMS -> Postgres -> Finance_DEV

### Metadata
Metadata -> Cloud -> AWS -> Redshift -> Dev -> Schema -> Table
...
## 6. Tab Header vs Content Rule

### Tab Header (Compact View)
Must include:
- Object icon
- Object name
- State indicator
...
Must NOT include full hierarchy.

Example:
Pipeline_Customer_Load  
*Pipeline_Customer_Load  

---

### Tab Tooltip (Full Context)
Hovering over a tab must display:
...
Pipeline_Customer_Load   [Running]  
Projects -> Project_A -> Folder_1 -> Pipeline_Customer_Load  
...
If duplicate object names exist:

- Tab label remains short
- Full path in tooltip disambiguates
- Optional minimal suffix may be added

Example:
Load_Data  
Load_Data (Finance)

Full path must always be available.

---

## 8. Navigation Behavior

Hierarchy nodes must be clickable.

Click behavior:
- Opens selected node in a new tab OR activates existing tab
- Must stay within ETL shell (no page navigation)

Example:
Click "Project_A" → opens Project tab

---

## 9. Execution & Audit Usage

Hierarchy path must be embedded in:
...
- Path must never be ambiguous
- Path must be consistent across all modules
- Path must not rely on color alone
- Path must support long names gracefully
- Path must be copyable

---

## 11. Acceptance Criteria

1. Every opened object shows full hierarchy in header
2. Tabs remain compact
3. Tooltip shows full path
4. Duplicate names are distinguishable
5. Navigation works via path clicks
6. Path format is consistent across all objects
7. Works for global and nested objects
